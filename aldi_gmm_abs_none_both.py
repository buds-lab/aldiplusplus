...
class ALDI():
    def __init__(self, 
                 ...
                 aldi_name='aldi_gmm_abs_none_both'):
    ...
    def plot_gmm_results(self, gmm):
        ...
        # Naming (aldi implementation | GMM data | GMM max components | GMM left components)
        figure.savefig(f'img/pD_evaluation/{self.aldi_name}_gmm-data-{self.gmm_data}_gmm-max-{self.gmm_max_comp}_site-{self.site_id}.png',
                       format='PNG')
        plt.clf()
    ...
    def get_result_df(self, count_left_comp=1, forecast_out=False):
        """
        ...
        sorted_gmm_components = self.gmm_components.sort_values(
                                                    'gauss_mean').copy()
        non_discord_gmm_component = sorted_gmm_components[:count_left_comp]
        ...
        if self.gmm_data == 'D':
            ...
            df_gmm_proba['is_discord'] = np.where(df_gmm_proba['max_comp'].isin(
                                    non_discord_gmm_component.component), 0, 1)
        else:
            gmm_proba = self.gm_model.predict_proba(
            ...
            # for the day to be classified as a "non-discord day"
            df_gmm_proba['is_discord'] = np.where(df_gmm_proba['max_comp'].isin(
                                    non_discord_gmm_component.component), 1, 0)
